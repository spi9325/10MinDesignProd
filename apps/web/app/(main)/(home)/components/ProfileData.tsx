...
export default function ProfileData({ authData, setProfile }: propType) {
  return (
    <div className=" absolute w-[100%] md:w-[400px] bg-white dark:bg-yellow-300 top-[70px] rounded-lg right-[0px] pb-[60px]">
      ...
      <div
        className={`mt-3 ${authData?.name?.length! < 6 ? "flex max-w-fit gap-2 mx-auto" : ""}`}
      >
        <p
          className={` max-w-fit px-2 font-bold font-serif text-center mx-auto dark:text-black`}
        >
          hi
        </p>
        {
          <p
            className={`${mainheading.className} mx-auto max-w-fit text-center font-medium dark:text-black`}
          >
            {authData?.name}
          </p>
        }
      ...
      <div className="flex max-w-fit gap-2 mx-auto">
        ...
        <p
          className={`${mainheading.className} w-fit mx-auto mb-5 dark:text-black`}
        >
          {authData?.email}
        </p>
      </div>
      ...
      <div className="w-[70%] mx-auto p-2 flex justify-center gap-4 cursor-pointer">
        <div
          id="profile-premium-btn"
          className="outline relative overflow-hidden p-2 w-[40%] text-center rounded-lg bg-black text-white"
        >
          Premium
        </div>
        <div
          ...
          className="outline p-2 w-[40%] text-center rounded-lg bg-black text-white flex justify-center"
        >
      ...
      <p
        ...
        className="p-2  mx-auto w-[40px] text-center rounded-full text-white cursor-pointer bg-red-600 mt-5"
      >
        X
      </p>
    </div>
